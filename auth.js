...
// Step 1 of password reset: email a 6-digit code. Always responds 200 even if the
// email doesn't exist, so this can't be used to find out which emails have accounts.
authRouter.post('/forgot-password', async (req, res) => {
  ...
  const code = generateCode();
  const expires = new Date(Date.now() + 15 * 60 * 1000).toISOString();
  db.prepare('UPDATE business SET reset_code = ?, reset_code_expires = ? WHERE id = ?').run(code, expires, business.id);

  sendEmailInBackground(email, 'Reset your Traveling Kids password', resetPasswordEmailHtml(code, business.contact_name));

  // Same reasoning as /register — always return the code as a fallback in case the
  // email is slow, fails, or lands in spam, so nobody gets locked out of their own account.
  res.json({ ok: true, devCode: code });
});

// Step 2 of password reset: check the code and set a new password.
authRouter.post('/reset-password', (req, res) => {
  ...
  const business = getBusinessByEmail(email);
  if (!business || !business.reset_code || business.reset_code !== code) {
    return res.status(400).json({ error: 'Incorrect or expired code.' });
  }
  if (new Date(business.reset_code_expires) < new Date()) {
    return res.status(400).json({ error: 'That code expired — request a new one.' });
  }

  db.prepare('UPDATE business SET owner_password_hash = ?, reset_code = NULL, reset_code_expires = NULL WHERE id = ?')
    .run(hashForStorage(newPassword), business.id);

...
import { getTripWithStops, updateStopStatus, triggerPanic } from './logistics.js';
...
// SOS button: logs an alert and notifies the business owner via WhatsApp + email
// immediately. Never fails outright — the alert is logged even if both notification
// channels are unreachable, so there's always a record.
driverRouter.post('/:token/panic', async (req, res) => {
  const data = getTripWithStops(req.params.token, true);
  if (!data) return res.status(404).json({ error: 'Trip link not found or expired.' });

  try {
    const notified = await triggerPanic(data.trip);
    res.json({ ok: true, notified });
  } catch (err) {
    console.error('Panic trigger failed:', err.message);
    res.status(500).json({ error: 'Could not send the alert. Call for help directly if this keeps failing.' });
  }
});

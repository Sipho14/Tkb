...
// Lets the owner assign/change which vehicle a driver uses, or edit their basic details.
adminRouter.patch('/drivers/:id', (req, res) => {
  ...
  if (vehicle_id !== undefined) { fields.push('vehicle_id = ?'); values.push(vehicle_id || null); }
  if (!fields.length) return res.status(400).json({ error: 'Nothing to update' });
...
adminRouter.get('/routes', (req, res) => {
  res.json(db.prepare(`
    SELECT r.*, d.name as driver_name, v.plate_number as vehicle_plate FROM routes r
    LEFT JOIN drivers d ON d.id = r.driver_id LEFT JOIN vehicles v ON v.id = d.vehicle_id
...
adminRouter.get('/routes/:id', (req, res) => {
  const route = db.prepare(`
    SELECT r.*, d.name as driver_name, d.phone as driver_phone, v.plate_number as vehicle_plate, v.id as vehicle_id
    FROM routes r LEFT JOIN drivers d ON d.id = r.driver_id LEFT JOIN vehicles v ON v.id = d.vehicle_id
    WHERE r.id = ? AND r.business_id = ?`).get(req.params.id, req.auth.businessId);
...
adminRouter.patch('/routes/:id', (req, res) => {
  ...
  if (!route) return res.status(404).json({ error: 'Not found' });
  const { delay_alert_threshold_minutes, driver_id, name, morning_time, afternoon_time } = req.body;
  const fields = [];
  const values = [];
  if (delay_alert_threshold_minutes !== undefined) { fields.push('delay_alert_threshold_minutes = ?'); values.push(delay_alert_threshold_minutes); }
  if (driver_id !== undefined) { fields.push('driver_id = ?'); values.push(driver_id || null); }
  if (name !== undefined) { fields.push('name = ?'); values.push(name); }
  if (morning_time !== undefined) { fields.push('morning_time = ?'); values.push(morning_time); }
  if (afternoon_time !== undefined) { fields.push('afternoon_time = ?'); values.push(afternoon_time); }
  if (!fields.length) return res.status(400).json({ error: 'Nothing to update' });
  values.push(req.params.id);
...
// Creates a trip for a route on a given date/leg (defaults to today + morning) and
// generates the driver's access link for it. Reuses an existing trip instead of
// duplicating one if it's already there for that route/date/leg.
adminRouter.post('/routes/:id/trips', (req, res) => {
  const route = db.prepare('SELECT * FROM routes WHERE id = ? AND business_id = ?').get(req.params.id, req.auth.businessId);
  if (!route) return res.status(404).json({ error: 'Route not found' });

  const service_date = req.body.service_date || new Date().toISOString().slice(0, 10);
  const leg = req.body.leg || 'morning';
  if (!['morning', 'afternoon'].includes(leg)) return res.status(400).json({ error: "leg must be 'morning' or 'afternoon'" });

  let trip = db.prepare('SELECT * FROM trips WHERE route_id = ? AND service_date = ? AND leg = ?')
    .get(route.id, service_date, leg);

  if (!trip) {
    const result = db.prepare('INSERT INTO trips (route_id, service_date, leg, status) VALUES (?, ?, ?, ?)')
      .run(route.id, service_date, leg, 'scheduled');
    trip = db.prepare('SELECT * FROM trips WHERE id = ?').get(result.lastInsertRowid);
  }

  const token = ensureDriverToken(trip);
  const driverUrl = `${req.protocol}://${req.get('host')}/driver.html?token=${token}`;
  res.json({ tripId: trip.id, serviceDate: service_date, leg, driverUrl });
});
...
// ---- Clients (parents) — onboarding/offboarding history ----
adminRouter.get('/clients', (req, res) => {
  res.json(db.prepare(`
    SELECT p.id, p.unique_id, p.name, p.whatsapp_number, p.home_address, p.created_at,
      p.active, p.deactivated_at,
      (SELECT COUNT(*) FROM students s WHERE s.parent_id = p.id AND s.active = 1) as active_students
    FROM parents p WHERE p.business_id = ? ORDER BY p.created_at DESC`).all(req.auth.businessId));
});

adminRouter.patch('/clients/:id/deactivate', (req, res) => {
  const parent = db.prepare('SELECT * FROM parents WHERE id = ? AND business_id = ?').get(req.params.id, req.auth.businessId);
  if (!parent) return res.status(404).json({ error: 'Not found' });
  db.prepare("UPDATE parents SET active = 0, deactivated_at = datetime('now') WHERE id = ?").run(parent.id);
  res.json({ ok: true });
});

adminRouter.patch('/clients/:id/reactivate', (req, res) => {
  const parent = db.prepare('SELECT * FROM parents WHERE id = ? AND business_id = ?').get(req.params.id, req.auth.businessId);
  if (!parent) return res.status(404).json({ error: 'Not found' });
  db.prepare("UPDATE parents SET active = 1, deactivated_at = NULL WHERE id = ?").run(parent.id);
  res.json({ ok: true });
});
